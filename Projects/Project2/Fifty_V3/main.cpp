...
//Version 3 with defaults, input, output files
...
int main(int argc, char** argv) {
    ...
    //Asking if they want to try the demo
    cout<<"Would you like to try the demo?"<<endl;
    cin>>help;
    if(help=='y'){
        demo();
        cout<<endl;
    ...
    cout<<"Would you like to play a real round with a partner? (y/n)"<<endl;
    ...
    while(input(inp)){
        ...
        out<<setw(15)<<left<<"PLAYER"<<setw(15)<<left<<"WIN RATE(%)"<<
                setw(15)<<left<<"EFFICIENCY(%)"<<setw(15)<<left<<"WINS"<<endl;
        out<<setw(15)<<left<<names[0]<<setw(15)<<left<<pwr[0]<<
                setw(15)<<left<<peff[0]<<setw(15)<<left<<p1w<<endl;

        out<<setw(15)<<left<<names[1]<<setw(15)<<left<<pwr[1]<<
...
void demo(int rounds){
    ...
    while(score<50){
        for(int i=0;i<rounds;i++){
            dice1=rand()%6+1;
            dice2=rand()%6+1;
            cout<<"Die 1: "<<static_cast<int>(dice1)<<endl
                <<"Die 2: "<<static_cast<int>(dice2)<<endl; //outputs die values
            //dice rolls must equal to each other in order to score
            if ( dice1==dice2 ){
                if ( dice1!=3 && dice2!=6 ){ //doesn't roll a pair of 3 or 6
                    score+=5;
                }
                else if ( dice1==6 ){ // rolls a pair of 6s
                    score+=25;
                }
            }
